<div class="flex flex-col gap-5">
    <div class="flex flex-wrap items-center justify-between gap-3">
        <div>
            <a href="{{ route('suprimentos.pedidos.index') }}" class="text-sm text-sky-700 hover:underline">← Todos os Pedidos</a>
    ...
    @unless ($isTerminal)
        <section aria-label="Operação" class="card flex flex-col gap-5">
            <div>
                ...
                <p class="text-sm text-slate-500">Cada alteração é persistida e registrada no histórico.</p>
            </div>

            <div class="grid grid-cols-1 gap-5 md:grid-cols-2">
                <form wire:submit="updateResponsavel" aria-label="Responsável" class="flex flex-col gap-2 rounded-lg border border-slate-200 p-4">
                ...
                <form wire:submit="updatePrioridade" aria-label="Prioridade" class="flex flex-col gap-2 rounded-lg border border-slate-200 p-4">
                ...
                <form wire:submit="updatePrevisao" aria-label="Previsão de entrega" class="flex flex-col gap-2 rounded-lg border border-slate-200 p-4">
                ...
                <form wire:submit="updateStatus" aria-label="Status" class="flex flex-col gap-2 rounded-lg border border-slate-200 p-4">
            ...
            <div aria-label="Cancelamento" class="flex flex-col gap-3 border-t border-slate-100 pt-4">
                @if (! $confirmingCancel)
                    <div class="flex flex-wrap items-center justify-between gap-3">
                        <p class="text-sm text-slate-500">O cancelamento é irreversível e fica registrado no histórico.</p>
                ...
                @else
                    <div role="alertdialog" aria-label="Confirmar cancelamento" data-testid="cancel-confirm-dialog"
                        class="flex flex-col gap-3 rounded-lg border border-red-200 bg-red-50 p-4">
                        <p class="text-sm font-medium text-red-800">Tem certeza que deseja cancelar este pedido? Esta ação é irreversível.</p>
